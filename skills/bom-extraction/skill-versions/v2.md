...
version: 2
status: current — live copy; snapshot preserved at skills/bom-extraction/skill-versions/v2.md
supersedes: v1.md (see skills/bom-extraction/skill-versions/ for immutable history)
...
| legend-dependent-tag (confirmed across 2 samples) | A tag's meaning depends on reading the drawing set's own legend/tagging sheet, not on generic convention for that drawing type. A project-specific prefix can otherwise be mis-classified. Confirmed on the P&ID sample (Legend1/Legend2) and the electrical one-line sample (GE01/GE02's tagging table). |
