...
note: >
  ...
  §11) — this same content is also placed into each of the three ELECTRICAL bidders' own
  reports/bid-evaluation/<doc>/report.md, per that use case's RANKED_COMPARISON shape.
skill-version: v6 (current live version; findings reformatted 2026-09-23 to v6's finding-format shape — see each document's graph.md; the underlying facts were unchanged by that reformatting)
