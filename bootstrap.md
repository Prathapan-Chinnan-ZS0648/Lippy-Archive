...
and supporting documents, not tied to any specific sample, document, or use case. Any
...
This terminology and the processing logic built on it hold regardless of use case, whether
the source is a contract revision, a report awaiting review against a template, a dataset
awaiting reconciliation against a prior period, or any other document-judgment task — the
skill resolved for that use case defines what "analyzed, compared, transformed, or
evaluated" concretely means; this file only defines the roles.
...
and adding a new format never requires editing this file. Format only determines *how* a
document's content is extracted into its twin (§12) — a paginated document's unit of
location is a page, a spreadsheet's is a sheet, a slide deck's is a slide, a tabular
file's is a row, and so on — never *whether* it can be processed at all. The extraction
approach for a given format is a `NORMALIZE`/`PLAIN` concern (§5), resolved dynamically
per document, not a rule that lives in this file.
...
- the path to the applicable skill file,
- and the human reviewer authorized to sign off on this use case's work
  (`signedBy`).

**One entry per use case.** `useCaseName` is the unique key across the list — two
entries never share a `useCaseName`. Switching which document a use case is currently
pointed at means editing that use case's own entry (its `sourceDocumentPath`/
`supportingDocumentPath`), never adding a duplicate entry for the same use case.
...
**`sourceDocumentPath` and `supportingDocumentPath` may each be a single path or a
...
hardcoded in this file — every concrete path referenced anywhere here is notation for
"wherever `fileIndex.md` currently points."
...
1. Read the source and supporting document location(s) from that entry's
   `sourceDocumentPath` and `supportingDocumentPath`.
...
3. Determine which use case/skill applies — read directly from that entry's
   `useCaseName` and `skillFilePath` fields (the skill version is whatever
   `skillFilePath`'s own front matter states — not a separate `fileIndex.md` field).
   Confirm `useCaseName` matches `skillFilePath`'s parent directory name under
   `skills/` before proceeding — see `RESOLVE` in §5.
4. Refer to the corresponding `skills/<usecase>/skill.md` (i.e. `skillFilePath`) for
   processing instructions.
5. Generate findings based on the applicable skill's instructions, once per resolved
   source/supporting pair.

Each entry holds **only these five fields** — `useCaseName`, `sourceDocumentPath`,
`supportingDocumentPath`, `skillFilePath`, `signedBy` — and nothing else, no
use-case-specific processing logic. (A field being list-valued per the pairing rules
above does not add another field — it is still exactly the same `sourceDocumentPath`/
`supportingDocumentPath` field, just holding more than one value.) `signedBy` names
the human reviewer authorized to sign off on this use case's work at the
use-case level — it does not itself record whether any specific document has been
signed, and never substitutes for the per-document `signed-by`/`verified-by` sign-off
that `manifest/<usecase>/manifest.md` records once HITL review actually completes for
that document (§15). Every other governance field (state, retention, classification),
sample context, and the digest ledger still live only in that use case's
`manifest/<usecase>/manifest.md`, never in `fileIndex.md`. See §13.
...
case; `fileIndex.md`'s `sourceDocumentPath`/`supportingDocumentPath` point into these
...
- **Derived from the skill, never invented independently.** The applicable skill's own
  file — its units/items, its questions or criteria, its groups/categories, its verdict
  vocabulary, its addressed/not-addressed (or absence-policy) logic, its evidence and
  citation requirements, its confidence levels, and any other evaluation-specific rule —
  is the sole authority for what a finding contains. `bootstrap.md` never hardcodes a
  unit list, a question, a verdict word, or a confidence level for any use case; a skill
  that needs a rule this file doesn't provide states that rule itself (§8.2's promotion
  bar governs how a skill's own rules grow, never this file growing to cover the gap).
  Where a finding states the question or item it is answering, that question is derived
  from the resolved skill's own stated intent and rules (its `intent` field, its Module
  1/2 rules, or an explicit per-item question list the skill defines) — never phrased
  independently of the skill and never a generic placeholder this file supplies on the
  skill's behalf.
- **Reviewer-style, human-auditable.** A finding reads as a human reviewer would document
  it after reading the evidence, not as a bare label: it states the unit/question being
  answered, the resulting verdict in the skill's own vocabulary, what the source
  documents actually say (quoted or precisely cited, never paraphrased into a claim the
  document doesn't make), and why that evidence leads to the verdict — reasoning kept
  visibly separate from quoted evidence, never blended so a reader cannot tell which is
  which.
- **Every citation precise enough to check.** Wherever the resolved document format
  supports it (a page, a section, a row, a sheet, a slide — whatever unit of location §1
  already establishes for that format), a finding names exactly where its evidence came
  from, and additional supporting evidence beyond the primary citation is named
  separately rather than folded into one undifferentiated citation.
- **No unsupported assumption, speculation, or invented evidence.** A finding never
  states something the source documents do not — the skill's own absence-policy (never
  this file's) governs what happens when evidence for a unit cannot be found.

The concrete front-matter fields and body section headings a finding uses (what a unit
is called, what the verdict field is named, what body sections appear and in what order)
are each use case's own skill's decision, per Module 5 (output requirements) of that
skill — never a fixed template this file imposes on every use case. Two use cases are
free to shape their findings completely differently (a bare verdict-and-evidence
paragraph for one, a labelled-section reviewer narrative for another, a table of
per-clause verdicts for a third) as long as each satisfies the four requirements above;
`bid-evaluation`'s and `version-compare`'s own findings (as they exist today) already do,
each in its own already-established shape, without needing to match each other's.
...
command beyond what that command's own definition composes). See non-negotiable rule 16.
...
3. Write a **HITL validation record** to
   `HITL/<usecase>/<source-document-name>/manualValidate.md`, containing at minimum:
   ...
   `<reviewer-name>` is always that use case's current `fileIndex.md` entry's
   `signedBy` field — never a fixed name written into this file (§2, §13). If
   `signedBy` is unset for the resolved entry, this step blocks with
   `<UNRESOLVED: signedBy not set for use case "<use-case-name>">` rather than
   guessing or leaving the field blank. `<execution-date>` and `<execution-time>` are
   always the actual date/time the command was executed — read at execution time, never
   hardcoded and never backfilled. `<use-case-name>` and `<document-name>` are always
   that use case's `useCaseName` and `<source-document-name>`, never a fixed example.
4. New validations for the same use case and document **append** to
   `HITL/<usecase>/<source-document-name>/manualValidate.md` rather than overwrite a
...
Read file-index
       ↓
Resolve Source Documents (RESOLVE, §5)
       ↓
...
Load Corresponding skill.md
       ↓
Normalize (NORMALIZE, §5) — build the twin layer: per-unit extraction, derived
summary, sectionMap.md, detection.md, plan.md
       ↓
Apply Skill Instructions (JUDGE, §5)
       ↓
Generate Findings
       ↓
...
`START` runs this flow to completion: every run enhances the skill against the sample it
just judged, not conditionally — a sample that surfaces nothing promotable still gets a
`patternLog.md` entry recording that (§8.1, §8.3) — and every run finishes by running
`MANUAL VALIDATE` for the sample it just produced output for, recording the HITL sign-off
before the run is considered complete. `PLAIN` stops after normalization (before "Apply
Skill Instructions"). `SKILL CHANGE <version_number>` re-enters partway through, reading
already-generated findings rather than regenerating them. `EXPORT` bypasses document
resolution entirely and operates on the configured directory as a whole (§5).
...
|---|---|
| Source document directory | `fileIndex.md`'s `sourceDocumentPath` |
| Supporting document directory | `fileIndex.md`'s `supportingDocumentPath` |
| File/document selection | `fileIndex.md` (the five configured inputs — nothing else is in scope, §3) |
| Use case | `fileIndex.md`'s `useCaseName` |
| Skill path | `fileIndex.md`'s `skillFilePath` |
| Skill version | the resolved skill file's own front matter `version` field — never duplicated into `fileIndex.md` |
| Authorized reviewer | `fileIndex.md`'s `signedBy` — used by `MANUAL VALIDATE`'s `<reviewer-name>` (§5) |
...
complete, accurate, and applicable across samples as more are processed — never merely
appended to, never left dependent on a single sample, and always applicable to both every
previously processed sample and every newly introduced one.
...
No `fileIndex.md` field needs updating for this — `fileIndex.md`'s `skillFilePath`
...
### 9.1 Version changelog

`skills/<usecase>/skill-versions/` must also hold a single, maintained file,
`skill-versions/versionChangelog.md`, giving a complete version-by-version account of
what changed between every consecutive pair of versions that use case's skill has ever
had — `v1` → `v2`, `v2` → `v3`, and so on through the current live version, never only a
comparison against `v1`. This file is generated/updated as part of step 2 above, whenever
a new version is created (a material change) or an existing version's own snapshot is
found to need correction — it is not optional documentation, it is part of what
completing a version bump means.

It must contain a table with at minimum these columns:

| Version | Previous version | Change | Type of change | Details / impact |
|---|---|---|---|---|

`Type of change` is one of: Added, Modified, Removed (or a combination, if a single
version bump did more than one of these). For every row, base the entry strictly on
what the two actual version snapshot files (`skill-versions/v<N-1>.md` and either
`skill-versions/v<N>.md` or the current live `skill.md`, whichever holds version `<N>`)
demonstrably contain — added instructions, removed instructions, modified instructions,
workflow changes, new or removed capabilities, input/output changes, validation changes,
processing or decision-logic changes, formatting/schema changes, new constraints or
rules, changes to domain-specific guidance, and changes to examples/templates where they
affect behavior all count as meaningful changes worth a row. Never invent a change or a
reason for a change that the two files don't actually support; if a version bump's
motivating reason isn't recorded anywhere retrievable (e.g. that version's own
`prompt-log/<usecase>/promptLog.md` entry per §9's step 3), describe only the observable
difference between the files, not a guessed rationale.
...
change must be justified by something true across every use case and every sample, not
one sample's needs. Since this file is not use-case-scoped, every amendment is recorded
as an entry in the `prompt-log/<usecase>/promptLog.md` of whichever use case's run
prompted it (or, if none did, the most recently active use case's log), recording what
changed and why, with the previous version diff-able from that entry.
...
├── fileIndex.md           exactly five inputs — use case name, source path, supporting path, skill path, signed-by reviewer — rewritten to point at whichever document/use case is currently being worked on
...
│           │   │                              summarized, mixed into, or otherwise represented here, regardless of the supporting document's own role or
...
│           └── versionChangelog.md   consecutive-version change table (§9.1) — the one
│                                      file in this directory that is maintained/updated
│                                      over time rather than written once and left immutable
│
├── reports/
│   └── <usecase>/
│       ├── <source-document-name>/report.md   ONLY where the resolved skill's canonical report shape (Module 5/output requirements) is genuinely
│       │                                      per-document — that document's own distinct content, not a copy of a comparison spanning other documents.
│       │                                      Exactly one report.md per such document — never multiple report files for the same document (amended v23,
│       │                                      §11). This file never names its shape (e.g. it never says "ranking summary" or "change report") — that
│       │                                      concrete meaning is defined per use case in that use case's own `skills/<usecase>/skill.md`, never here.
...
per-use-case isolation as every other generated root (§16 rule 17): each use case accumulates its
...
and its `useCaseName` determines both which `documents/<usecase>/` subtree it is placed
...
exactly `useCaseName`, `sourceDocumentPath`, `supportingDocumentPath`,
`skillFilePath`, `signedBy` — exactly these five fields, nothing else, though
`sourceDocumentPath` and `supportingDocumentPath` may each be list-valued within an
entry per §2's pairing rules. `signedBy` is a name only — the reviewer authorized for
this use case — never a status, date, or verification record; those stay in
`manifest/<usecase>/manifest.md`'s per-document sign-off (§15). No skill version, no
classification, no terminology notes, no digests, no use-case-specific processing logic.
It must not restate or override the skill's judging rules — it only points at the
skill(s).
...
5. Historical skill versions (`v<N>.md` snapshots) in `skills/<usecase>/skill-versions/`
   are immutable; `versionChangelog.md` in that same directory (§9.1) is the one
   exception, maintained/updated as new versions are created.
...
19. Every `.md` filename created anywhere in this project follows **lowercase-first
    camelCase** (e.g., `bootstrap.md`, `fileIndex.md`, `patternLog.md`, `sectionMap.md`,
    `manifest.md`, `promptLog.md`, `skill.md`, `detection.md`, `plan.md`, `graph.md`,
    `base.md`, `record.md`, `score.md`, `report.md`, `readMe.md`), including files
    under `skills/`, `actuals/`, `findings/`, `reports/`, `HITL/`, `manifest/`, and
    `prompt-log/` — amended v22, §11, superseding the capital-first CamelCase this rule
    stated through v18. `HITL/<usecase>/<source-document-name>/manualValidate.md` (§5's
    `MANUAL VALIDATE` record) is no longer a named departure from this rule — it already
    used this same lowercase-first pattern before the rest of the project's filenames
    were brought in line with it, and is now simply one ordinary instance of it, not an
    exception.

    The single exception is a filename derived directly from a source or supporting
    document's own identifier (e.g., `twin/derived/<DocName>.md`, where `<DocName>`
    mirrors the real document's name, or a per-unit twin file whose name embeds the
    document's own section/sheet/slide title) — that identifier keeps its own casing
    verbatim, since it names a specific document or the document's own structure rather
    than describing project structure.

    This rule governs filenames only; it never requires rewriting the historical content
    of any `promptLog.md`, `patternLog.md`, or `skills/<usecase>/skill-versions/`
    snapshot — those may still refer to a filename by whatever casing was live at the
    time that entry or snapshot was written.
