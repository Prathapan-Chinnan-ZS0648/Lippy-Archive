...
| 1 | UNDERSTAND | Read every callout of the source sheet, 22 units, by area (title block / notes / plan view) and grid position | The Grid D midpoint-column node becomes units `MEMB-4` through `MEMB-7` |
...
22 units in total: 4 ADDED, 15 CHANGED, 3 UNCHANGED (control cases), 0 REMOVED — 3 of the CHANGED units (`MEMB-10`–`MEMB-12`) are `confidence: UNSURE`, confirmed changed by the text-layer diff but not yet independently pinned to a grid row; see `pivot.md` § 4. `MEMB-13`–`MEMB-16` and `CTRL-3` were added 2026-09-17 after an independent second pack (`sample-2`) flagged that this pack's first pass missed a real dimension change at (and a second occurrence of the same node pattern beyond) `CTRL-1`; see `pivot.md` § 8. See `actuals/report/report.md` for the full row-by-row breakdown and `actuals/twin/derived/` for the per-document callout lists this plan operates over.
